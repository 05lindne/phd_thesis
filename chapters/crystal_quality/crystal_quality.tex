...
\chaptermark{Quality}
		...
		Carbon exists in a variety of crystalline and disordered structures, exhibiting three hybridizations, sp$^3$, sp$^2$, sp$^1$, see  \cref{fig::c_bonds}. In its sp$^3$ configuration, characteristic of diamond, each of the four carbon valence electrons are assigned to a tetrahedrally directed sp$^3$ orbital resulting in strong $\sigma$ bonds with adjacent atoms.
		...
		\section{Raman Measurements}
			...
			\\
			For Raman measurements the setup described in \cref{ch::confocal_setup} is used.
		...
		\section{Transmission Electron Microscopy}{\label{sec::tem}}
			...
			\subsection{Crystallinity and Grain Boundaries}\label{subsection::tem_crystal}
				...
				\\
				In \cref{fig::tem_crystal} a \TEM image of a single \nd is shown. A complex crystal formation deviating from single crystal diamond structure is visible. A higher resolution detail is given in \cref{subfig::tem_boundary}. The image clearly shows different crystal orientations, asserting that the \nd particle is not a single crystal diamond. As a result, the imaged \nd is of reduced crystal quality.
				\\
