...
\section{Coupling \sivs to \Vcsels} \label{sec::coupling_vcsel}
	...
	The optical output power of \BmFour as a function of the current applied is given in \autoref{subfig::vcsel_output_power}. In addition, the resulting voltages are shown. It can be seen that the maximum power of $\approx \SI{0.6}{\mW}$ is reached for moderate currents of $\approx \SI{4.5}{\mA}$.

	\begin{remark}
			Die plots zeigen, dass maximal 1 mW an optischer leistung da aus dem \VCSEL rauskommt. Im confocalen setup wird aber mit wesentlich mehr angeregt, 25 mW is da schon drin. Warum ist das kein problem, dass die leistung vom \VCSELs so gering ist?
	\end{remark}
	...
	\begin{figure}[htp]
		\begin{subfigure}[t]{ 0.49\linewidth}
			\centering
			\testbox{\includegraphics[trim = 0 0 0 0,  clip= true, width = \textwidth]{./pics/field330_scanxy06_x19y15_34uW_t30_2_fit_lines.pdf}}
			\caption{}
		...
		\begin{subfigure}[t]{ 0.49\linewidth}
			\centering
			\testbox{\includegraphics[trim = 0 0 0 0,  clip= true, width = \textwidth]{./pics/spe_scan_xy-25_300uW_t60_700-800nm_lines.png}}
			\caption{}
	...
	\begin{figure}[tp]
		...
		\caption{Reflectivity of the \dbr (DBR) of the VCSEL, and spectrum of the \siv measured during VCSEL excitation. The reflectivity of the DBR and the VCSEL emission spectra are depicted with different scales. The shape of the measurement of the \siv during VCSEL operation coincides with the shape of the DBR reflectivity. The spectrum of the \siv is not visible. As the emission from the \siv is small compared to the intensity of the laser sideband in the same wavelength regime, the \sivs emission is not detectable\todo[inline]{warum schaut das spectrum anders aus als das, was einzeln geplottet ist?}.}
		\label{fig::dbr_vcsel}
	\end{figure}
	...
	Another line at \SI{0}{nm} (denoted line B) \todo{check number} which was a minor feature in the spectrum before \pp, is the predominant line after the process.
	...
	While the energy of the electrons is low compared to the ionization energy of the color center \todo{numbers}, we observed a reduction of \fl intensity after electron radiation (for a detailed description of this effect, refer to \cite{alexmeyer::baccthesis}\todo{besser begruenden, was passiert}.
	\\
	We then operated the VCSEL at \SI{1.84}{mA} and \SI{3.3}{V} and turned off the laser of the confocal setup.
	We scanned both the surface of VCSEL Bm4 (with \nd) and of VCSEL Bm2 (without \nd) with a \SIrange{730}{750}{nm} bandpass filter.
	This filter window suppresses the VCSEL laser line at \SI{655}{nm} (\ref{subfig::vcsel_output_wavelength}) while leaving the \siv emission nearly unchanged.
	The light areas in \cref{fig::vcsel_excitation} correspond to the laser output areas.
	We measured a spectrum at the same points as before VCSEL operation.
	In the case of VCSEL Bm4, this spot corresponds to the position of the \siv.
	...
	Hence, we conclude, that the detected emission is due to the VCSEL and does not stem from the \siv\todo{ev besser begruenden}
