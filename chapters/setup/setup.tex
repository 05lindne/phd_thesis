...
\chaptermark{Experimental Setup}
	...
	%
	% In this context, two capabilities are particularily key: Exciting \sivs into emmiting fluerecent light using a Laser and the ability to register \siv flourescence down to single-photon resolution.
	%
	% In this chapter we detail a home-build confocal setup on \fl which elegantly combines both task.
	...
	\section[Confocal Setup]{Confocal Setup} \label{sec::confocal}
		...
		After this cleanup filter, the excitation beam hits a \SI{}{\milli\meter} glass plate (fabricator Halle Germany \todo[fancyline]{thickness}) which redirects the beam. It is then focused onto the sample by a $100 \times$ microscope objective (Olympus, LMPlanFLN) with a numerical aperture of $0.8$.
